...
int main(int argc,char** argv){
	...
	const int img_length=1280;
	const int img_width=720;
	const int conv_kernal_size=3;
	const int pl_kernel_num=12;
	...
	unsigned int result_length=img_length-conv_kernal_size+1;
	unsigned int result_width=img_width-conv_kernal_size+1;
	//AIE filter2D result
	int aie_result[result_length*result_width];
	memset(aie_result,0,sizeof(int)*result_length*result_width);
	...
	int img[img_length*img_width];
	for(int i=0;i<img_length*img_width;++i) img[i]=i;

	//compute the input and output data length
	int inputDataLength[pl_kernel_num],outputDataLength[pl_kernel_num];
	inputDataLength[0]=(img_width/pl_kernel_num+1)*img_length;
	inputDataLength[pl_kernel_num-1]=inputDataLength[0];
	outputDataLength[0]=(img_width/pl_kernel_num-1)*result_length;
	outputDataLength[pl_kernel_num-1]=outputDataLength[0];
	for(int i=1;i<pl_kernel_num-1;++i){
		inputDataLength[i]=(img_width/pl_kernel_num+2)*img_length;
		outputDataLength[i]=(img_width/pl_kernel_num)*result_length;
	}
	...
	for(int i=1;i<pl_kernel_num;++i){
		...
		memcpy(DataInput[i],img+((img_width/pl_kernel_num)*i-1)*img_length,inputDataLength[i]*sizeof(int));
	...
	int* ps_result=filter2D(img,img_width,img_length,conv_kernel,conv_kernal_size);
	...
	int errorCount=0;
	for(unsigned int i=0;i<result_length*result_width;++i){
